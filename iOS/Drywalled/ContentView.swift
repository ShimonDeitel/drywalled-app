...
struct ContentView: View {
    ...
    @State private var editingEntry: PatchEntry?
...
struct EntryFormView: View {
    ...
    @State private var stage: String = PatchEntry.stageOptions.first ?? ""
    ...
    var body: some View {
        NavigationStack {
            Form {
                Section("Details") {
                    ...
                    Picker("Stage", selection: $stage) {
                        ForEach(PatchEntry.stageOptions, id: \.self) { Text($0).tag($0) }
                    }
            ...
            .navigationTitle(mode.id == "add" ? "New Patch" : "Edit Patch")
            .toolbar {
    ...
    private func save() {
        switch mode {
        case .add:
            let entry = PatchEntry(title: title, stage: stage, date: date, note: note, photoData: photoData)
            store.add(entry)
        case .edit(var entry):
            entry.title = title
            entry.stage = stage
